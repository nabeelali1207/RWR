...
% pdws.RF_V = double(~pdws.RF_V);
% pdws.RF = double(floor(((pdws.RF) / 100 - 2000) / (0.244)));
% pdws.RF = pdws.RF .* ~pdws.RF_V;
% plot(pdws.TOA * 50e-9, 100 * (0.244* pdws.RF +2000), '.m' ) % RF

pdws.TOA = double(floor(pdws.TOA/5));
plot(pdws.TOA(1: end - 1) * 50e-9, diff(pdws.TOA(rfv == 1)) ./ 20, ':.r', 'MarkerSize',22); % PRI

% plot(ans.TOA * 50e-9, ans.PW * 2.5, '.k') % PW
